...
def player_stats():
    ...
    if stat_type == 'highest_checkout':
        ...
        stats = db.session.query(
            ...
            func.max(PlayerGameWeekStats.highest_checkout).isnot(None).desc(),
        ...
        for p in all_players:
            ...
            player_data.append({
                ...
                'gw_data': gw_data,
                'total': total,
                'has_stats': len(p.game_week_stats) > 0
            })
        # Sort by has_stats desc (True first), then total desc, then name asc
        player_data.sort(key=lambda x: (not x['has_stats'], -x['total'], x['name']))
